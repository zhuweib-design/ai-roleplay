...
describe('角色属性 V2 导入 (F01.6)', () => {
  test('从 data.extensions.attributes 导入完整属性', () => {
    const v2Json = {
      spec: 'chara_card_v2',
      spec_version: '2.0',
      data: {
  ...
  test('无 extensions 字段时 attributes 为 undefined', () => {
    const v2Json = {
      spec: 'chara_card_v2',
      spec_version: '2.0',
      data: { name: 'Test' },
  ...
  test('extensions 无 attributes 字段时 attributes 为 undefined', () => {
    const v2Json = {
      spec: 'chara_card_v2',
      spec_version: '2.0',
  ...
  test('全空属性时 attributes 为 undefined', () => {
    const v2Json = {
      spec: 'chara_card_v2',
      spec_version: '2.0',
      data: {
...
describe('角色属性 V2 导出 (F01.6)', () => {
  test('导出时 attributes 写入 data.extensions.attributes', () => {
  ...
  test('无 attributes 时不写入 extensions', () => {
    const card = makeCard();
    const v2 = exportV2Card(card);
    const ext = v2.data.extensions as Record<string, unknown> | undefined;
  ...
  test('保留已有 extensions 字段并合并 attributes', () => {
    const card = makeCard({
...
describe('角色属性提示词注入 (F01.6)', () => {
  test('属性区块注入到系统提示词', () => {
    const card = makeCard({ attributes: makeAttributes() });
    const result = buildPrompt(card, emptyHistory, '你好', makeSettings());
