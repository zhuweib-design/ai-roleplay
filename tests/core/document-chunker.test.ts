...
describe('文档分块器 (F09.1)', () => {
  ...
  describe('超长段落切分', () => {
    ...
    test('默认 maxChunkLength 为 MAX_CHUNK_LENGTH (2000)', () => {
      const text = '短文本';
      const chunks = chunkDocument(text);
      expect(chunks).toHaveLength(1);
  ...
  describe('buildChunks 元数据生成', () => {
    test('为分块生成 ID 和序号', () => {
      ...
      const chunks = buildChunks('doc-1', rawChunks);
      expect(chunks).toHaveLength(3);
    ...
    test('空数组返回空', () => {
      const chunks = buildChunks('doc-1', []);
