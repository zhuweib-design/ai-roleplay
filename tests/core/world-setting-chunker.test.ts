...
describe('chunkWorldSetting (需求 1:超 512 token 分块)', () => {
  it('短设定:单块,标题不变,首行标题', () => {
    const chunks = chunkWorldSetting('w1', '精灵王国', '精灵王国位于大陆东部的翡翠森林之中。', 512);
  ...
  it('超长设定:多块 + 续块标题', () => {
    ...
    const chunks = chunkWorldSetting('w2', '精灵王国', body, 512);
    expect(chunks.length).toBeGreaterThan(1);
    expect(chunks[0].title).toBe('精灵王国');
    expect(chunks[1].title).toBe('精灵王国(续 2)');
    // 每块 token 不超预算(标题前缀少量溢出容忍:标题 + 正文 ≤ 512 + 标题 token)
    for (const c of chunks) {
      expect(countTokens(c.text)).toBeLessThanOrEqual(512 + countTokens(c.title));
    }
  });

  it('空输入返回空数组', () => {
    expect(chunkWorldSetting('w3', 'T', '')).toEqual([]);
    expect(chunkWorldSetting('w3', 'T', '   ')).toEqual([]);
  });
});