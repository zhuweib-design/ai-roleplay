...
### Phase 4：边界与体验（✅ 已完成 2026-08-15）
- [x] 4.1 快速滚动防重入：`windowLoading` 双向防重入（Phase 3 已覆盖）；**容器宽度缓存**避免滚动加载循环反复读 `clientWidth` 强制 reflow（实测 5000 条帧率 31→51fps）
- [x] 4.2 消息跳转（减量）：新增**"回到底部"浮动按钮**（视口非底部时显示，`atBottom` 状态 + `jumpToBottom` 扩展窗口并滚底，aria-label 双语）；完整消息搜索/锚点跳转登记为后续增强
- [x] 4.3 键盘可达性：浮动按钮 `aria-label` + `focus-visible` 环；虚拟化不破坏 skip-link / focus outline / 消息区 `role=log`（审查确认）
