...
| 5 | 🟡 P2 | CSP/remote 白名单收紧 | 盘点+决策记录：`P2-9-csp-whitelist-report.md`——**保留"任意自定义端点"开放定位**（产品核心卖点，收敛即破坏）；边界已覆盖：Rust `validate_endpoint` 禁私网/link-local（SSRF）、CSP `connect-src https://*` 禁 http 明文、`script-src 'self'` 禁远程代码、capabilities 最小权限（无 fs/shell/http 插件）、市场 https 严格校验；残余风险（https 外发，用户配置驱动）记录 threat-model §4 | ✅ 已盘点并决策 |
| 6 | 🟡 P2 | 人工视觉冒烟（需 GUI/发布机）| **本环境已自动化完成**：全 17 路由 Playwright 无头截图核验（scripts/ui-shot.mjs，1.7 MB+15 PNG）；**修复 2 处 UI 问题**：① ImageGeneratorView 未配置时增加醒目红色引导横幅（"需要先添加模型才能生成图像" + 前往设置按钮）；② SettingsView 左侧 Tab 描述改为 2 行 line-clamp（"云端/本地模型与 API 配置"完整展示，原单行 nowrap 被无声截断）；i18n imgGen.configWarning/goToSettings zh/en | vue-tsc 0 错；i18n strict 通过；lint 0 errors；全量 2574/2574 | ✅ 已修复 |
