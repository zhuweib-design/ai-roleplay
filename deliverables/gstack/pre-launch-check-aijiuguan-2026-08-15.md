...
| 5-6 | 🟡 P2 | CSP/remote 白名单收紧（需产品决策）、人工视觉冒烟（需 GUI/发布机）| 计划已纳入 `P2-11-virtualization-plan.md` §2 | ⏳ 待决策/待环境 |
| 11 | 🟡 P2 | 长对话虚拟化 | **Phase 1-3 已完成**：基线压测（改造前上滚 20 次 DOM 2100 气泡/40953 节点、内存 92MB）→ 方案选型（自研双向窗口，无新依赖）→ 核心实现（ChatMain `[windowStart,windowEnd]` + 顶/底 spacer + 高度估算/缓存 + 双向滚动锚定 + windowLoading 防重入）；**改造后上滚 20 次 DOM 恒 200 气泡/3905 节点（-90.5%）、内存 61MB（-34%）**；Phase 4 边界体验 / Phase 5 压测验收待续（计划：`P2-11-virtualization-plan.md`）| 🟢 Phase 1-3 完成 |
